...
function fetchPosts (): Promise<Post[]> {
    return axios.get(`${BASE_URL}/posts`).then((response: AxiosResponse<{ data: Post[] }>) =>
        response.data.data
    );
}

function fetchPost(id: string): Promise<Post> {
    return axios.get(`${BASE_URL}/posts/${id}`).then((response: AxiosResponse<{ data: Post }>) =>
        response.data.data
    );
}

function searchPosts(search: string): Promise<Post[]> {
    return axios.post(`${BASE_URL}/posts/search/`, {
        search,
    }).then((response: AxiosResponse<{ data: Post[] }>) =>
        response.data.data
    );
}
...
function deletePost(id: number): Promise<void> {
    return axios.delete(`${BASE_URL}/posts/${id}`).then((response: AxiosResponse) => response.data);
}

export { fetchPosts, fetchPost, searchPosts, createPost, updatePost, deletePost };